...
export default class DateCalculator {
  ...
  static getStartOfFuturestack() {
    let now = new Date();
    let tomorrow = new Date(2022, 4, 17);
    return tomorrow;
  }
  ...
  static getExampleDateString() {
    ...
    }/${exampleDate.getDate()}/${exampleDate.getFullYear()}`;
  }

  static getEndDateFromDateOption(dateOption) {
    switch (dateOption.timeUnit) {
      case FUTURESTACK.timeUnit:
        return this.getStartOfFuturestack();
      case TODAY.timeUnit:
